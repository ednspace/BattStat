...
class MainFrame(wx.Frame):
    ...
    def load_graph(self):
        ...
        self.subplot.set_title(r"Battery Status")
    ...
    def draw_graph(self):
        ...
        self.subplot.plot(self.seconds, self.BattV, 'r-', linewidth = 1)
        self.canvas.draw()
    ...
    def OnLoad(self,EVENT):
        self.SetStatusText("Loading Battery Data...")
        self.SensorTimer.Stop()
        self.filename = None
        ...
        for line in file.readlines():
            if (skip >= 3):
                ...
                fields = line.split ('\', \'')
                self.BattV.append(fields[1])
                paren = line.rfind(')')
                self.seconds.append(line[paren-19:paren-1])
            skip = skip + 1
        
        self.seconds = map(float, self.seconds)
        self.BattV = map(float, self.BattV)
